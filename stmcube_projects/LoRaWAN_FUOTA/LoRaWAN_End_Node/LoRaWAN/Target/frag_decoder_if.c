...
void FRAG_DECODER_IF_OnProgress(uint16_t fragCounter, uint16_t fragNb, uint8_t fragSize, uint16_t fragNbLost)
{
  ...
  APP_LOG(TS_OFF, VLEVEL_ALWAYS, "EXPECTED   : %03d Fragments (%05d Bytes)\r\n", fragNb, fragNb * fragSize);
  APP_LOG(TS_OFF, VLEVEL_ALWAYS, "RECEIVED   : %03d Fragments\r\n", fragCounter - fragNbLost);
  APP_LOG(TS_OFF, VLEVEL_ALWAYS, "             %05d Bytes\r\n", (fragCounter - fragNbLost) * fragSize);
  APP_LOG(TS_OFF, VLEVEL_ALWAYS, "LOST       : %03d Fragments\r\n\r\n", fragNbLost);
  if (fragCounter > fragNb)
  {
    APP_LOG(TS_OFF, VLEVEL_ALWAYS, "REDUNDANCY : %03d Fragments\r\n", fragCounter - fragNb);
  }
...
void FRAG_DECODER_IF_OnDone(int32_t status, uint32_t size, uint32_t *addr)
{
  ...
  APP_LOG(TS_OFF, VLEVEL_ALWAYS, "STATUS     : %d\r\n", status);
  ...
  //*addr = FRAG_DECODER_DWL_REGION_START; REMOVED BY CLEM
#else
  uint32_t FileRxCrc = Crc32(UnfragmentedData, size);
  APP_LOG(TS_OFF, VLEVEL_ALWAYS, "Size       : %05d Bytes\r\n", size);
  APP_LOG(TS_OFF, VLEVEL_ALWAYS, "CRC        : %08X\r\n\r\n", FileRxCrc);
  //*addr = (uint32_t)&UnfragmentedData[0]; REMOVED BY CLEM
